...
const ViewDetails = () => {
  ...
  const { id } = useParams();
  const { role } = useRole(); // role from hook
  const isLoggedIn = !!role;
  const canApply = role && role.toLowerCase() === 'borrower';
  ...
  if (isError || !loan)
    return (
      <p className="text-center py-20 text-xl">Error fetching loan details.</p>
    );

  // Theme colors (match AllLoans page)
  ...
  return (
    <div className={`${bgColor} min-h-screen py-16 px-6 md:px-20 transition-colors duration-500`}>
      <motion.div
      ...
      >
        ...
        <div className="md:flex md:gap-6">
          ...
          <div className={`md:w-1/2 p-10 flex flex-col justify-center ${cardText}`}>
            <motion.h1
            ...
            >
              <p>
                <span className={`font-semibold ${labelColor}`}>Category:</span> {loan.loan_category}
              </p>
              <p>
                <span className={`font-semibold ${labelColor}`}>Interest Rate:</span> {loan.interest_rate}%
              </p>
              <p>
                <span className={`font-semibold ${labelColor}`}>Max Loan Limit:</span> ${loan.max_loan_limit.toLocaleString()}
              </p>
            </motion.div>
            ...
            >
              ...
              <ul className="list-disc ml-6 mt-2 space-y-1">
                {Array.isArray(loan.available_emi_plan) &&
                  loan.available_emi_plan.map((plan, idx) => <li key={idx}>{plan}</li>)}
              </ul>
            </motion.div>
            ...
            <motion.div className="mt-6">
              {!isLoggedIn ? (
                <button
                  className="px-3 py-2 rounded-md btn-primary w-full font-semibold mt-auto"
                ...
                <button
                  disabled
                  className={` px-3 py-2 rounded-md btn-primary w-full font-semibold mt-auto opacity-60 cursor-not-allowed`}
                >
